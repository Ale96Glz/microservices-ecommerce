package com.aosorio.ecommerce.notificaciones.security;

import com.aosorio.ecommerce.notificaciones.exception.AccessDeniedException;
...
public final class GatewayAuth {
    ...
    public static void requireSelfOrAdmin(User user, Long resourceUserId) {
        if (user.isAdmin()) {
            return;
        }
        if (resourceUserId == null || !user.id().equals(resourceUserId)) {
            throw new AccessDeniedException("No puedes acceder a recursos de otro usuario");
        }
    }
}
